function [eff_sig] = effstress_of_StateMises3D(Mdata,mdata,sig)
...
faeta=Mf\rightTemp';
eta=(sig.xx+sig.yy-2*sig.zz)/(MisesPlaneStress(sig));
...
sigxx=sig.xx-sig.zz;
sigyy=sig.yy-sig.zz;
sigxy=sqrt(sig.xy^2+sig.yz^2+sig.xz^2);

effstress=sqrt(sigxx^2+sigyy^2-sigxx*sigyy+3*sigxy^2);
